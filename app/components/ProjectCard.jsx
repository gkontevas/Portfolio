...
const ProjectCard = ({ imgUrl, title, description, gitUrl, previewUrl }) => {
  return (
    <div className="flex flex-col h-full rounded-lg shadow-xl overflow-hidden bg-[#1a1a1a] transition-all duration-300 hover:shadow-2xl hover:-translate-y-2">
      <div
        className="h-36 sm:h-48 md:h-60 relative group"
        style={{
      ...
      >
        <div className="absolute inset-0 bg-black bg-opacity-0 flex items-center justify-center opacity-0 group-hover:opacity-100 group-hover:bg-opacity-80 transition-all duration-500">
          <Link
            href={gitUrl}
            className="h-10 w-10 sm:h-12 sm:w-12 flex items-center justify-center border-2 border-gray-500 rounded-full bg-opacity-40 hover:border-white hover:bg-gray-800 transition-all duration-300"
          >
            <CodeBracketIcon className="h-6 w-6 sm:h-8 sm:w-8 text-gray-300 hover:text-white" />
          </Link>
          {/* <Link
            href={previewUrl}
      ...
      <div className="p-3 sm:p-4 md:p-5 bg-gray-800 text-white flex flex-col flex-grow">
        <h5 className="text-lg sm:text-xl font-semibold mb-1 sm:mb-2 text-gray-200">{title}</h5>
